...
router.put('/todo/:id' , function(req, res){
    Todos.updateOne({_id: req.params.id}, { todo : req.body.todo}, {date: req.body.date}, function(err, doc){
        if(err)  throw err;
...
router.put('/check/:id', function (req, res) {
    ...
    Todos.updateOne({ _id: req.params.id }, { check: req.body.check }, {date: req.body.date}, function (err, doc) {
        if (err) throw err;
